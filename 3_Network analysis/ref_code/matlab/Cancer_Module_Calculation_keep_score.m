function Cancer_Module=Cancer_Module_Calculation_keep_score(Cancer_Type,alpha,cutoff);
...
load(['../Raw_Module/Raw_Module_',Cancer_Type,'_',str]); %%%%% module
...
module_gene=cell2mat(Module(score(:,4))); % genes within a modele
umg=unique(module_gene);

%%%%%%% Cancer_Module
Cancer_Module=[];
for i=1:length(umg)
    l=length(find(module_gene==umg(i)));% gene confidence value
    umg(i,2)=l;
end
umg=sortrows(umg,-2);
l=find(umg(:,2)<cutoff*length(score),1,'first');
if l>0
    Cancer_Module=umg(1:l,:);
else 
    Cancer_Module=umg(:,:);%select all genes
end
