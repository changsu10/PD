...
load ../GPSnet_input/PPI.mat;   %%%%% network
load(['../GPSnet_input/',Cancer_Type]);  %%%%% mutation
load(['../GPSnet_input/',Cancer_Type,'_gene_length_df.mat']); %%%%% gene length
...
for i=1:LL
    ...
    if mod(i,1)==0
        toc;
        disp(i)
        if ~exist('../Raw_Module/', 'dir')
            mkdir('../Raw_Module/')
        end
        save(['../Raw_Module/Raw_Module_',Cancer_Type,'_',Str_alpha,'.mat'],'Module','Score')
    end
end
